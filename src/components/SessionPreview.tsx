  ...
  return (
    <div className="space-y-4 font-mono text-sm">
      <div>
        ...
        {warmup.map((s, i) => (
          <div key={i} className="flex gap-4 text-zinc-400 pl-2">
      ...
      <div>
        ...
        {main.map((s, i) => (
          <div key={i} className="flex gap-4 text-zinc-100 pl-2">
            <span className="w-16 text-right">{s.weight}lb</span>
            <span>x {s.reps}{s.isAmrap ? '+' : ''}</span>
            {s.isAmrap && <span className="text-amber-400 text-xs">AMRAP</span>}
          </div>
        ))}
      </div>
      <div>
