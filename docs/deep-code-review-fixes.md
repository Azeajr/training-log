...
5. **Mobile claims follow the project rule** — an iOS/bfcache fix is not `fixed`
   on this machine's evidence alone. F67 was the live case; a device test settled it
   on 2026-09-16 and its severity was amended down as a result.
...
| F09 | Medium | B04 | — | `src/lib/export-import.ts` | `fixed` | `38d375e` · `export-import.test.ts` ×4 | A timed or distance accessory records its performance in **neither weight nor reps**, and the CSV had no column for either — so a plank and a farmer's walk exported as blank rows and the only figure those movements have was silently discarded. The JSON backup kept it the whole time, which is what made the loss invisible. `duration_s` and `distance_m` now sit between `reps` and `is_amrap`; the five pinned row shapes moved with them. |
...
| F11 | Medium | B04 | — | `src/store/settings-store.ts` | `fixed` | `03425c8` · `settings-store.test.ts` ×3 | `loadSettings` **returned early** when there was no settings row, so an import carrying no settings table left the previous install's values live in memory — bar weight, plates, cycle shape, supplemental template — over a database that had none of them, until something happened to reload the page. It follows the database down to defaults now. Second half: a restored theme was written into the store and never painted, so the screen kept the old palette while the store already reported the new one — `applyTheme` is called on every load, not only from `updateSettings` and boot. |
| F12 | Medium | B05 | — | `src/store/workout-store.ts` | `fixed` | `03425c8` · `workout-store.test.ts` ×5 | The persistence effect called `localStorage.setItem` with no catch, on the render path, so a quota or write failure threw out of a reactive effect with nothing above it. **The silence is the cost:** logged sets are safe in the database, but the cursor, the accessory work and the session notes live only in this store until COMPLETE, so a reload after that point loses them. Caught, reported through `persistenceError`, and Workout says so — naming what is still safe, what is not, and that finishing now writes everything. |
...
| F36 | Low | B07 | — | `src/lib/training-max.ts` | `fixed` | `debafe7` · `training-max.test.ts` ×2 | B12d decision applied: highest `id` wins at equal `setAt`. One `isNewer` helper shared by `getCurrentTm` and `getAllCurrentTms`, which previously resolved the same tie to opposite rows. |
| F37 | Medium | B07 | — | `src/lib/pr.ts` | `fixed` | `5e51256` · `pr.test.ts` cross-history case | Not in the planned chunk — fixed as a consequence of the shared reader. The "no history at all" guard tested the lift's **own** session rows and returned before the cross query ran, so a movement trained entirely as cross work was scored against nothing. It is now decided on every qualifying session, cross-only included. Both previously pinned behaviours preserved. |
| F38 | Medium | B07 | — | `src/lib/pr.ts` | `fixed` | `5e51256` · `pr.test.ts` ×4 | The three readers now share one rule via `lib/performance.ts`. The live-session clause is what lets the toast and the History badge agree while the toast still works mid-workout — `pr.ts:78-84` asserted that invariant and did not hold it. |
| F39 | Medium | B07 | — | `src/lib/tm-recommendations.ts` | `fixed` | `f68a39e` · `tm-recommendations.test.ts` ×8 | **Provenance recorded instead of inferred.** Whether a training max came from auto-progression or from the user was read off a 60-second wall clock around the cycle's creation, and the CYCLE COMPLETE modal writes TMs immediately afterwards — so the same data and the same action landed on either side of the window according to dwell time. Tapped at 10s the lift stayed a doubling candidate; tapped at 61s it was silently disqualified a whole cycle later. `trainingMaxes` gains `source` and `cycleId`; only `'manual'` counts as a bump, and the timestamp now only places a row relative to the cycle's own start. Legacy rows carry null and keep the old inference — for those the provenance genuinely is unknown, and guessing would be worse than saying so. The review's own probe delays (10s, 59s, 61s, 5min) are asserted to give one answer. |
| F40 | Low | B07 | — | `src/lib/tm-recommendations.ts` | `fixed` | `f68a39e` · `tm-recommendations.test.ts` archived case | `getCycleDoublingCandidates` read `db.lifts.toArray()` and never filtered `archived`, while the progression it feeds goes through `activeLiftsOrdered`. An archived lift with three qualifying weeks was offered a doubled increment it can never take. Same population as the progression now. |
| F41 | Low | B07 | C1 | `src/lib/exercise.ts` | `fixed` | `13ad7e5` · `exercise.test.ts` ×3 | Application rule became a storage invariant: `idx_exercises_name_nocase` on `exercises(TRIM(LOWER(name)))`. Placed in **`ADDITIVE_MIGRATIONS`, not `SCHEMA`** — `init()` runs `SCHEMA` unguarded on *every* boot, so a DB already holding duplicates would fail to start; as a migration the error is swallowed and that DB skips the index. Same reasoning as `idx_accessoryNotes_session_exercise`. The constraint failure is translated back to `ExerciseNameConflictError` so a lost race reads like a deliberate duplicate. |
| F42 | Low | B07 | — | `src/lib/exercise-history.ts` | `fixed` | `5e51256` · `exercise-history.test.ts` ×2 | `getLiftHistory` matched the lift's **own** sessions only, so cross work logged on another lift's day was invisible — while counting toward the same lift's PR toast, Stats record and AMRAP seed. Now resolved through `baselineSets`. |
| F43 | Medium | B07 | — | `src/lib/lift.ts` | `fixed-by` | `5e51256` · reader half only | **Reader half only.** `db.sets.where('liftId')` had no session join, so sets orphaned by `archiveLift` scored permanent records no screen could display; `baselineSets` resolves cross work through its session and drops orphans. **`archiveLift`'s missing cascade is NOT fixed** — that is a separate defect and stays open as F99. |
| F44 | Low | B07 | — | `src/lib/lift.ts` | `fixed` | `c979b65` · `lift.test.ts` ×2 | `deleteLift` now completes the cascade: the lift's sessions and all their child rows, plus its `assistanceDefaults`. "Hard-delete a lift and everything attached to it" has to mean it — the doc comment scoped the function to pre-history use, but nothing enforced that. |
| F45 | Low | B07 | — | `src/lib/cleanup.ts` | `fixed` | `c979b65` · `cleanup.test.ts` ×4 | **Configuration counts as use.** "In use" meant "has a surviving logged set", so CLEANUP archived an exercise the user had just set as a lift's assistance default and given a training max — the slot then resolved to nothing. A live `assistanceDefaults` reference or an accessory TM now protects it, and the plan carries `exerciseNamesToArchive` so the confirmation can say what it is about to do rather than reporting a bare count afterwards. |
| F46 | Low | B08 | — | `src/components/modals/ModalAsyncStates.tsx` | `fixed` | `38d375e` · `ModalAsyncStates.test.tsx` ×6 | Three independent predicates over a `list()` that collapsed error and loading onto the same value — null whenever `error` was set **or** the query was in flight — so an errored sheet rendered the error message *and* a permanent "Loading..." underneath it, telling the user both that it had failed and that it had not finished. One derived `State` and a `Switch`: a single state cannot say two things at once. The file had **no test at all**; one of the six asserts exactly one state renders across every combination of inputs. |
| F47 | Low <br><sub>med w/F34</sub> | B08 | — | `src/components/modals/CycleCompleteModal.tsx` | `fixed` | `debafe7` · `CycleCompleteModal.test.tsx` focus case | `initialFocus="container"` on `CycleCompleteModal`. The rule for that prop is now "first control performs a write", not "first control says DELETE" — the first focusable was `+X LBS`, which armed a training-max write under the next Enter keypress. |
| F48 | Medium | B08 | — | `src/hooks/use-confirmation.ts` | `fixed` | `3201c50` · `use-confirmation.test.ts` ×12 | **A hang that produced no error.** `confirmWithChoice` stored the new request's `resolve` over the old one with no queue and no settlement, so a second `confirm()` while one was pending left the first promise permanently unsettled — and every caller awaits it, so everything after that await never ran. Settings has eight call sites; Today and Workout gate abandon, redo, skip and exit on one. The displaced request now resolves `'cancel'`: what Escape already means, and the honest reading — a confirmation the user never saw has not been agreed to, so every `if (!await confirm(...))` bails exactly as it would have. The file had no test; the new ones race each promise against a timer and report `UNSETTLED`, because a promise that never settles cannot be asserted on directly, which is the same reason the bug was invisible. |
| F49 | Low | B08 | C3 | `src/components/modals/LiftSetupModal.tsx` | `fixed` | `60a3e98` · `LiftSetupModal.test.tsx` ×3 | A `loaded` flag gates the equipment controls, following the modal's own `saving()` precedent. The form rendered interactive while showing **defaults**, and `load()` then replaced `plateMode`/`implementBase` with the stored values — so a choice made in that window was silently undone, and a lift stored as `none` displayed the wrong mode until the query landed. `ToggleChip` and `Stepper` gained a `disabled` prop. `LiftSetupModal` had no test file. |
| F50 | Medium | B08 | — | `src/components/modals/LiftSetupModal.tsx` | `fixed` | `3201c50` · `LiftSetupModal.test.tsx` ×4 | `handleCommit` had a try/finally and no catch, and its button called it as `void handleCommit()`. A rejected transaction went nowhere: `onCommit` never fired, SAVING… reverted to DONE, and the dialog sat there refusing to close with nothing explaining why. Caught and shown now, with the buffered edits left untouched — the point of buffering is that a failed write costs the write, not the work — and the button re-enabled, because a stuck SAVING… is its own dead end. |
| F51 | Medium | B08 | C1,C2 | `src/components/workout/SaveFailureBanner.tsx` | `fixed` | `13ad7e5` · `SaveFailureBanner.test.tsx` ×3 | `retrying` is a set of ids, not one `number | null` slot. It tracked in-flight state for a **list**, so retrying B re-enabled A while A was still in flight, and whichever settled first cleared the marker for both — on the one path whose purpose is recovering a set already lost once. |
...
| F53 | Low <br><sub>cosmetic</sub> | B08 | — | `src/components/workout/AmrapTargets.tsx` | `fixed` | `38d375e` · inspection, not a test | Labels padded with `padEnd(14)` to line the rep counts into a column, emitted as ordinary HTML text with no `whitespace-pre` anywhere above them — so the browser collapsed every run of spaces to one and the column never existed. The padded label is now its own `whitespace-pre` span. **Deliberately not tested:** `textContent` keeps the spaces either way, so a jsdom assertion cannot tell the fixed state from the broken one. Verified by inspecting the class list, as the finding itself prescribes. |
...
| F55 | Medium | B08 | C1,C2 | `src/components/workout/AccessoryPicker.tsx` | `fixed` | `13ad7e5` · `AccessoryPicker.test.tsx` ×2 | Both commit paths single-flight via `useSingleFlight`, and `alreadyAdded` is now derived **live** from `workout.activeAccessories` rather than snapshotted into `rows()` at load time, so the guard can see an add made by the previous tap. |
| F56 | Low | B08 | C3 | `src/components/workout/AccessoryLog.tsx` | `fixed` | `60a3e98` · `AccessoryLog.test.tsx` ×3 | Closed from both sides. The log controls are withheld until `props.exercise` resolves — `type()` fell back to `'reps'`, so logging in that window wrote `reps: n, duration: null` for a **timed** exercise. And `exercises` is now fetched with the session and lift rather than on the last await, since it depends on nothing ahead of it while `workout.activeAccessories` is hydrated synchronously from localStorage. |
...
| F58 | Medium | B08 | C4 | `src/components/forms/Stepper.tsx` | `fixed` | `0e3ba10` · `Stepper.test.tsx` ×5 | Two independent stops, each with its own test. The repeat now ends **from inside** when the clamped value stops changing — a button disables itself exactly when the value pins at its bound, and a disabled button dispatches no pointer events, so nothing outside could ever stop it. Release is also listened for on the **window**, so a pointer that wanders off the control still ends the press. `Stepper.test.tsx` had 17 cases and none touched the long-press path. |
| F59 | Low | B08 | C5 | `src/components/forms/DurationInput.tsx` | `fixed` | `b2e0af5` · `AccessoryLog.test.tsx`, `DurationInput.test.tsx` ×2 | `fieldLabel` passed at all three call sites. It existed for exactly this — "so two duration inputs on one screen don't both announce as bare minutes/seconds" — and none of them used it. Tested at the **call site**, not just the component: a component-level test passes whatever the call sites do. |
| F60 | Medium <br><sub>WCAG 2.1.2, Level A</sub> | B08 | C5 | `src/components/forms/NotesField.tsx` | `fixed` | `b2e0af5` · `NotesField.test.tsx` ×4 | Tab interception gated on `listMode()`, as the Enter handler already was. **Behaviour change, taken deliberately:** Tab re-tabbing is now a list-mode feature, so three existing tests were updated to enable list mode. WCAG 2.1.2 permits a non-standard exit only if the user is advised of it, and the ←/→ escape chips render only in list mode — so outside it there was no advice and no exit. |
| F61 | Medium <br><sub>WCAG 2.1.1, Level A</sub> | B08 | C5 | `src/components/forms/SetReadout.tsx` | `fixed` | `b2e0af5` · `SetReadout.test.tsx` ×7 | A real `<button>` when the row is tappable. This is the app's only affordance for editing an already logged set, so as a bare `div` correcting a mislogged set was pointer-only. The trailing slot stays **outside** the button — it holds an `InlineConfirm`, and nesting interactive content makes the inner control unreachable. `SetReadout` had no test file. |
...
| F64 | Low | B08 | C5 | `src/components/layout/Rule.tsx` | `fixed` | `b2e0af5` · `Rule.test.tsx` ×5 | The dash fill is wrapped in `aria-hidden` inside `Rule`, so all 16 call sites get it — the fix that had been applied once at a call site now lives in the component. `Modal` keeps its own `aria-hidden`, but for a different reason now: its `sr-only` heading is the `aria-labelledby` target, so without it the title is announced twice. |
...
| F67 | Low <br><sub>was Medium</sub> | B09 | C6 | `src/lib/notifications.ts` | `fixed` | `985fb2a` · `notifications.test.ts` ×2 · settled on device 2026-09-16 | `firePage` wraps the constructor and falls back to `ServiceWorkerRegistration.showNotification`. Unwrapped, the `TypeError` escaped the timer tick uncaught: no notification, and nothing reporting that none fired — while this module designates the page as the **reliable** path. An explicit permission denial is still respected rather than routed around. **Settled 2026-09-16 on an installed iOS PWA.** The constructor is available there: the Settings toggle reaches `granted`, and a notification is constructed and displayed. The inversion this finding feared does not apply on the platform it named, so the severity is amended Medium → Low in `deep-code-review.md` (rule 4). The fallback stays — probe P27 showed what a refusing engine produces — but it now guards an engine class no targeted platform is known to be in. The same device test opened **F107** against the visible-tab abstain in this file. |
| F68 | Low | B09 | — | `src/workers/timer.worker.ts` | `fixed` | `38d375e` · `rest-timer-protocol.test.ts` ×4 | `resume` cleared the paused flag and posted nothing, and the 1 Hz interval kept its original phase, so the first `elapsed` after the tab became visible arrived up to a full second late — while the countdown on screen had already jumped to the true value. The timer visibly disagreed with itself for that second. It posts immediately now and restarts the interval so the phase resets. |
| F69 | Medium | B10 | — | `vite.config.ts` | `fixed` | `e0f0356` · `pnpm test:coverage` green on ratcheted thresholds | `include` widened from three directories to `src/**`. `components`, `db`, `hooks`, `service-worker.ts` and `workers` were **not measured at all** — 22 of the 23 findings this review opened in those areas lived where the gate could not see. Measured over the whole tree the real figures are **88.79 / 78.27 / 88.18 / 91.17**, so thresholds are re-baselined to **85 / 76 / 85 / 88** — just under actual, so the gate bites on every metric instead of leaving three with ~9 points of slack. |
| F70 | Medium <br><sub>needs Lighthouse</sub> | B10 | — | `vite.config.ts` | `fixed` | `9d90447` · `pwa-manifest.test.ts` ×7 | **The delivery mechanism, failing silently.** The manifest declared `icon-192.png` and `icon-512.png` and neither file existed, so Chrome's installability criteria — a manifest icon of at least 144×144 that actually loads — were never met and the install prompt never appeared; `index.html` had no `apple-touch-icon`, so iOS had no home-screen icon either. The build succeeded and CI passed the whole time, because nothing held the declaration against the filesystem. New PNGs generated from the SVG, an apple-touch-icon link, and the manifest moved to `src/pwa-manifest.ts` as data so a test can read it: every declared icon must be a real non-empty file, one must be ≥144px, one must be maskable, and each PNG's **IHDR dimensions must match its declared `sizes`** — a regenerated icon at the wrong size would otherwise still "exist". PNGs committed rather than built, so CI needs no image toolchain; `scripts/generate-icons.mjs` reproduces them through Playwright's bundled Chromium. Adding the module to `tsconfig.node.json` is **F71's gate working** — the typecheck refused the import until the file belonged to a project. Lighthouse not run: the assertion that the icons resolve is the thing that was missing. |
| F71 | Medium | B10 | — | `tsconfig.json` | `fixed` | `e0f0356` · deliberate type errors caught in both projects | `tsconfig.e2e.json` added to the solution file's `references`, and `tsconfig.node.json` widened to `playwright.config.ts`, `eslint.config.js`, `stryker.config.mjs` and `scripts/**`. Verified rather than assumed: a deliberate type error in `tests/e2e/app.spec.ts` and another in `playwright.config.ts` are both now caught by `tsc -b`, and neither was before. |
| F72 | Low | B10 | — | `vite.config.ts` | `fixed` | `9d90447`, `be40b92` · `precache.test.ts` ×9 | **(a)** `globPatterns` omitted `svg`, so favicon.svg — the only icon that existed — was not precached and failed offline. `PRECACHE_PATHS` derives from that manifest, so one entry fixes it rather than special-casing the path in the SW. **(b)** The orphaned `icons.svg` sprite is deleted. **The follow-up is the interesting part:** vite-plugin-pwa also adds the manifest's icons to the precache list, so an icon matching both sources was listed twice — and `cache.addAll()` rejects with `InvalidStateError` on a repeated URL. The service worker then failed to **install**: no page controlled, no offline shell, all seven hardening legs red at once. Caught by `pnpm verify:sw` while the unit suite, the build and the E2E suite were all green — exactly that job's purpose. `dedupePrecacheUrls`/`toPrecachePaths` extracted to `lib/precache.ts` so the SW's one piece of list logic is testable in milliseconds. |
...
| F74 | Medium | B10 | — | `.github/workflows/ci.yml` | `fixed` | `e0f0356` · new `e2e` CI job | A third CI job runs `test:e2e` on every PR and every push to main, uploading the Playwright report on failure. Landed **with** F85 and F78 — wiring a failing suite into CI, or one that tests the dev server, returns it to exactly the state that produced these findings. |
| F75 | Low | B10 | — | `.github/workflows/ci.yml` | `fixed` | `e0f0356` · `push` trigger added | `push: branches: [main]` added, with the concurrency group falling back to `github.ref` since `pull_request.number` is empty on a push. Committing on main is an accepted workflow here, and it previously got **no** checks unless the commit happened to touch a deploy path. |
...
| F77 | Low | B10 | — | `eslint.config.js` | `fixed` | `e0f0356` · 0 → 64 rules on `scripts/*.js` | A `**/*.{js,mjs}` block extending `js.configs.recommended` with node globals. `eslint --print-config scripts/debug-browser.js` resolved **0** rules before and **64** now; ~400 lines of Playwright-driving Node were checked by nothing. Passes clean. |
| F78 | Medium | B10 | — | `playwright.config.ts` | `fixed` | `e0f0356` · 32/32 against a production build | `webServer` is `pnpm build && vite preview` on 5175, not `pnpm dev`. **`freshStart` no longer needs `__e2eResetDb`** — Playwright gives every test its own context, and a context has its own storage partition, which is already a clean install and is exactly what `verify-notify-hardening.js` relies on. Exposing the reset hook in production was the alternative and was rejected: it is a destructive global. |
...
any leverage in it. Every `open` finding had to belong to exactly one batch —
`batches.test.ts` still enforces that, so a finding reopened later cannot
quietly land outside the plan, and it also checks that every recorded commit
SHA is reachable from HEAD.

Batch membership was a judgement call and was revised twice: batch 1 was pulled
ahead of 3 to get a working gate behind the riskiest work, and batch 6 ahead of
5 because it held the only remaining finding that broke something for every
user every time.
...
**The two hangs neither reported anything, because neither produced an error.**
`confirmWithChoice` left a displaced promise permanently unsettled, so every
caller's code after `await confirm(...)` simply never ran; `handleCommit`
discarded a rejected transaction, so a dialog sat there refusing to close. An
unsettled promise and a swallowed rejection are both invisible to every
mechanism that reports problems, which is why they survived a review that found
99 other things.

**F39 and F100 were one finding.** Both wanted provenance on `trainingMaxes`,
and the ledger said so before the work started; doing them together cost one
migration instead of two. F39 was also the sharpest thing in this batch: a
lift's eligibility for a doubled increment turned on **how long the user took to
tap a button**, a whole cycle earlier, with nothing on screen to explain it.

One fix is deliberately untested. F53's collapsed padding cannot be settled in
jsdom — `textContent` keeps the spaces whether or not `whitespace-pre` is
applied — so it is verified by inspecting the class list, which is what the
finding itself prescribes. The row says so.
