...
| `open` | **68** |
| `wip` | 0 |
| `fixed` | **31** — F02, F04, F05, F06, F22, F33, F34, F36, F37, F38, F41, F42, F47, F51, F52, F54, F55, F57, F63, F58, F59, F60, F61, F64, F65, F66, F73, F79, F95, F96, F97 |
...
| F58 | Medium | B08 | C4 | `src/components/forms/Stepper.tsx` | `fixed` | `<pending>` · `Stepper.test.tsx` ×5 | Two independent stops, each with its own test. The repeat now ends **from inside** when the clamped value stops changing — a button disables itself exactly when the value pins at its bound, and a disabled button dispatches no pointer events, so nothing outside could ever stop it. Release is also listened for on the **window**, so a pointer that wanders off the control still ends the press. `Stepper.test.tsx` had 17 cases and none touched the long-press path. |
